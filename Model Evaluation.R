...
extract_lm_table <- function(model, outcome_label) {
...
save_diag_plot <- function(model_y0, model_y1, filename_prefix) {
  png(
  ...
  tryCatch(
    plot(model_y0, which = 1, main = "Residuals vs Fitted — logged tumor in mRNA expression"),
    error = function(e) { plot.new(); title("Residuals vs Fitted — logged tumor in mRNA expression") }
  )
  
  tryCatch(
    plot(model_y0, which = 2, main = "Normal Q-Q — logged tumor in mRNA expression"),
    error = function(e) { plot.new(); title("Normal Q-Q — logged tumor in mRNA expression") }
  )
  
  tryCatch(
    plot(model_y1, which = 1, main = "Residuals vs Fitted — logged tumor in RhB biodistribution"),
    error = function(e) { plot.new(); title("Residuals vs Fitted — logged tumor in RhB biodistribution") }
  )
  
  tryCatch(
    plot(model_y1, which = 2, main = "Normal Q-Q — logged tumor in RhB biodistribution"),
    error = function(e) { plot.new(); title("Normal Q-Q — logged tumor in RhB biodistribution") }
  )
...
calculate_metrics_phys <- function(outcome_name, train_data, test_data) {
  ...
  suppressWarnings({
    ...
    X_train <- model.matrix(as.formula(formula_str), data = train_data)[, -1]
    y_train <- train_data[[outcome_name]]
    X_test  <- model.matrix(as.formula(formula_str), data = test_data)[, -1]
    ...
    lm_pred <- tryCatch(predict(lm_model, test_data), error = function(e) rep(NA, nrow(test_data)))
    lasso_pred <- tryCatch(as.numeric(predict(lasso_model, X_test)), error = function(e) rep(NA, nrow(test_data)))
    enet_pred <- tryCatch(as.numeric(predict(enet_model, X_test)), error = function(e) rep(NA, nrow(test_data)))
    rf_pred <- tryCatch(as.numeric(predict(forest_model, test_data)), error = function(e) rep(NA, nrow(test_data)))
    ...
    xgb_pred <- tryCatch(as.numeric(predict(xgb_model, as.matrix(X_test))), error = function(e) rep(NA, nrow(test_data)))
    ...
    svm_pred <- tryCatch(as.numeric(predict(svm_model, test_data)), error = function(e) rep(NA, nrow(test_data)))
...
perform_cv_phys <- function(df, outcomes, n_folds = 5) {
  ...
  fold_ids <- sample(rep(1:n_folds, length.out = length(compound_ids)))
  compounds_folds <- data.frame(compound_id = compound_ids, fold = fold_ids)
  ...
  for (outcome_name in outcomes) {
    ...
    for (fold in 1:n_folds) {
      ...
      fold_metrics <- calculate_metrics_phys(outcome_name, train_data, test_data)
      cv_metrics[[fold]] <- fold_metrics
    }
...
calculate_metrics_comp <- function(outcome_name, train_data, test_data) {
  ...
  suppressWarnings({
    ...
    svm_pred <- tryCatch(as.numeric(predict(svm_model, X_test)), error = function(e) rep(NA, nrow(test_data)))
    
    lm_pred <- tryCatch(as.numeric(predict(lm_model, test_data)), error = function(e) rep(NA, nrow(test_data)))
...
perform_cv_comp <- function(df, outcomes, n_folds = 5) {
  ...
  for (outcome_name in outcomes) {
    ...
    for (fold in 1:n_folds) {
      ...
      test_data <- df[df[[compound_col]] %in% test_compounds, ]
      train_data <- df[!df[[compound_col]] %in% test_compounds, ]
      fold_metrics <- calculate_metrics_comp(outcome_name, train_data, test_data)
      cv_metrics[[fold]] <- fold_metrics
    }
...
# =========================================================
# 1) PHYSICOCHEMICAL LINEAR MODEL
# =========================================================
model_y0_phys <- lm(
  logged_tumor_mrna_expression ~ Amines_in_Lipid_scale + Size_scale + Zeta_scale + PDI_scale + EE_scale,
  data = data_model
)

model_y1_phys <- lm(
  logged_tumor_rhb_biodistribution ~ Amines_in_Lipid_scale + Size_scale + Zeta_scale + PDI_scale + EE_scale,
  data = data_model
)

phys_lm_table <- bind_rows(
  extract_lm_table(model_y0_phys, "logged tumor in mRNA expression"),
  extract_lm_table(model_y1_phys, "logged tumor in RhB biodistribution")
)

write.csv(
  phys_lm_table,
  "Model Evaluation/physicochemical_linear_model_final_table.csv",
  row.names = FALSE
)
...
save_diag_plot(
  model_y0_phys,
  model_y1_phys,
  "Model Evaluation/physicochemical_linear_model_diagnostics.png"
)
...
# =========================================================
# 2) COMPOSITION LINEAR MODEL
# =========================================================
model_y0_comp <- lm(
  logged_tumor_mrna_expression ~ Ionizable + Phospho + Helper + PEG,
  data = data_model
)

model_y1_comp <- lm(
  logged_tumor_rhb_biodistribution ~ Ionizable + Phospho + Helper + PEG,
  data = data_model
)

comp_lm_table <- bind_rows(
  extract_lm_table(model_y0_comp, "logged tumor in mRNA expression"),
  extract_lm_table(model_y1_comp, "logged tumor in RhB biodistribution")
)

write.csv(
  comp_lm_table,
  "Model Evaluation/composition_linear_model_final_table.csv",
  row.names = FALSE
)
...
save_diag_plot(
  model_y0_comp,
  model_y1_comp,
  "Model Evaluation/composition_linear_model_diagnostics.png"
)
...
message("Done.")
message("Saved files:")
message("Model Evaluation/physicochemical_linear_model_final_table.csv")
message("Model Evaluation/physicochemical_model_evaluation_metrics.csv")
message("Model Evaluation/physicochemical_linear_model_diagnostics.png")
message("Model Evaluation/composition_linear_model_final_table.csv")
message("Model Evaluation/composition_model_evaluation_metrics.csv")
message("Model Evaluation/composition_linear_model_diagnostics.png")